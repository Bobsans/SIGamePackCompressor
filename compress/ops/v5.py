...
def optimize_files(source: ZipFile, dest: ZipFile, bs: BeautifulSoup, queue: Queue):
    ...
    if logo := package.get('logo'):
        try:
            ...
            queue.put_nowait(f'Logo optimized! [{format_size(len(original_content))} -> {format_size(len(content))}]')
        except Exception as e:
            queue.put_nowait(f'Logo optimization error! {e}')
    ...
    for tag in [*images, *videos, *audios]:
        ...
        if type == 'image':
            try:
                ...
                queue.put_nowait(f'Image {name}{ext} [{format_size(len(original_content))}] -> {hashname}{ext} [{format_size(len(content))}]')
            except Exception as e:
                queue.put_nowait(f'Image optimization error! {e}')
        elif type == 'video':
            try:
                original_content = get_file_content(source, filename, type)
                ...
                queue.put_nowait(f'Video {name}{ext} [{format_size(len(original_content))}] -> {hashname}{ext} [{format_size(len(content))}]')
            except Exception as e:
                queue.put_nowait(f'Video optimization error! {e}')
        elif type == 'audio':
            try:
                original_content = get_file_content(source, filename, type)
                ...
                tag.string.replace_with(f'{name}{ext}')
                queue.put_nowait(f'Audio {name}{ext} [{format_size(len(original_content))}] -> {hashname}{ext} [{format_size(len(content))}]')
            except Exception as e:
                queue.put_nowait(f'Audio optimization error! {e}')
